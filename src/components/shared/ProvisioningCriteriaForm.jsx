...
const ProvisioningCriteriaForm = ({ initial, onSubmit, submitting }) => {
  ...
  const [selectedLoanProductIds, setSelectedLoanProductIds] = useState(
    new Set()
  );
    ...
    (async () => {
      ...
      try {
        ...
        const exp =
          ...
          [];
        const prods =
          d?.loanProducts || d?.loanProductOptions || d?.products || [];
  ...
  useEffect(() => {
    ...
    const lpIds = Array.isArray(initial.loanProductIds)
      ? initial.loanProductIds
      : [];
    ...
    const normRows = Array.isArray(initial.entries)
      ? initial.entries.map((r) => ({
          ...
          provisioningPercentage: Number(
            r.provisioningPercentage ?? r.percentage ?? 0
          ),
  ...
  const validate = () => {
    ...
    rows.forEach((r, i) => {
      ...
      if (r.minAge === "" || r.minAge === null || isNaN(Number(r.minAge)))
        e[`rows.${i}.minAge`] = "Min Age is required";
      if (r.maxAge === "" || r.maxAge === null || isNaN(Number(r.maxAge)))
        e[`rows.${i}.maxAge`] = "Max Age is required";
      if (Number(r.minAge) > Number(r.maxAge))
        e[`rows.${i}.range`] = "Min must be ≤ Max";
      if (
        r.provisioningPercentage === "" ||
        r.provisioningPercentage === null ||
        isNaN(Number(r.provisioningPercentage))
      )
        e[`rows.${i}.pct`] = "Percentage is required";
  ...
  const lpIndex = useMemo(
    () => new Map(loanProductOptions.map((p) => [p.id, p.name])),
    [loanProductOptions]
  );
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (errors.criteriaName)
                  setErrors((x) => ({ ...x, criteriaName: "" }));
              }}
              placeholder="e.g. Standard Loan Loss Provisioning"
              className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
            />
            {errors.criteriaName && (
              <p className="text-xs text-red-500 mt-1">{errors.criteriaName}</p>
            )}
          </div>
        </div>
      </Card>
      ...
      <Card>
                ...
                <div className="grid md:grid-cols-3 gap-3 mt-3">
                  <div>
                    ...
                    <select
                      value={r.categoryId}
                      onChange={(e) =>
                        setRow(idx, { categoryId: e.target.value })
                      }
                      className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                    {errors[`rows.${idx}.maxAge`] && (
                      <p className="text-xs text-red-500 mt-1">
                        {errors[`rows.${idx}.maxAge`]}
                      </p>
                    )}
                    {errors[`rows.${idx}.range`] && (
                      <p className="text-xs text-red-500 mt-1">
                        {errors[`rows.${idx}.range`]}
                      </p>
                    )}
                ...
                <div className="grid md:grid-cols-3 gap-3 mt-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setRow(idx, { provisioningPercentage: e.target.value })
                      }
                      className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                    {errors[`rows.${idx}.pct`] && (
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setRow(idx, { liabilityAccount: e.target.value })
                      }
                      className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                    >
                      <option value="">Select…</option>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setRow(idx, { expenseAccount: e.target.value })
                      }
                      className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
                    >
                      <option value="">Select…</option>
        ...
        {errors.rows && (
          <p className="text-xs text-red-500 mt-2">{errors.rows}</p>
        )}
      </Card>
        ...
        <details className="group">
            ...
            {loanProductOptions.length ? (
              loanProductOptions.map((lp) => (
                ...
                >
                  ...
                  />
                  <span className="truncate" title={lp.name}>
                    {lp.name}
                  </span>
                </label>
              ))
              ...
              <div className="text-xs text-gray-500">
                No loan product options available.
              </div>
            )}
          </div>
          {!!selectedLoanProductIds.size && (
            <p className="text-[11px] text-gray-500 mt-2">
              Selected:{" "}
              {Array.from(selectedLoanProductIds)
                .map((id) => lpIndex.get(id) || id)
                .join(", ")}
            </p>
          )}
        </details>
