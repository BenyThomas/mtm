...
const CashMovementModal = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const d = r?.data || {};
        const cur = d?.currencyOptions || d?.currencies || d?.currency || [];
        const pay = d?.paymentTypeOptions || d?.paymentTypes || [];
        const curList = Array.isArray(cur) ? cur : [];
        const payList = Array.isArray(pay) ? pay : [];
        if (!cancelled) {
          setCurrencyOptions(
            curList
              .map((c) => c.code || c.currencyCode || c.name || c.value)
              .filter(Boolean)
          );
          setPaymentTypes(
            payList
              .map((p) => ({
                id: p.id ?? p.value ?? p.key,
                name: p.name ?? p.text ?? p.label ?? `#${p.id}`,
              }))
              .filter((x) => x.id)
          );
        }
  ...
  const title =
    mode === "settle" ? "Settle Cash From Cashier" : "Allocate Cash To Cashier";
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        ...(txnDate
          ? { txnDate, dateFormat: "yyyy-MM-dd", locale: "en" }
          : { locale: "en" }),
      ...
      footer={
        <>
          ...
          <Button onClick={submit} disabled={busy}>
            {busy ? "Processing…" : mode === "settle" ? "Settle" : "Allocate"}
          </Button>
        </>
      }
    >
