...
const ReportMailingJobForm = ({ initial, onSubmit, submitting }) => {
  ...
  const formatOptions = useMemo(() => {
    ...
    const fromTpl = Array.isArray(list)
      ? list.map((x) => x?.name || x?.value || x).filter(Boolean)
      : [];
  ...
  const weekdayOptions = useMemo(() => {
    ...
    if (Array.isArray(fromTpl) && fromTpl.length)
      return fromTpl.map((x) => String(x).toUpperCase());
    return [
      "MONDAY",
      "TUESDAY",
      "WEDNESDAY",
  ...
  useEffect(() => {
    ...
    setReportName(
      initial.reportName || initial.reportTitle || initial.report || ""
    );
    const rp = Array.isArray(initial.reportParameters)
      ? initial.reportParameters
      : [];
    setReportParams(
      rp.length
        ? rp.map((p) => ({
            name: p.name || p.reportParameterName || p.parameterName || "",
            value: p.value ?? "",
          }))
        : [{ name: "", value: "" }]
    );
    ...
    setAttachmentFormat(
      initial.emailAttachmentFileFormat || initial.attachmentFormat || "PDF"
    );

    // Start date/time: accept 'YYYY-MM-DD HH:mm' or ISO
    const sd =
      initial.startDateTime || initial.startDatetime || initial.startAt;
    if (sd) {
      const s = String(sd);
    ...
    const freq = (initial.frequency || initial.recurrence?.frequency || "")
      .toString()
      .toUpperCase();
    if (freq) setFrequency(freq);
    setInterval(
      Number(initial.interval || initial.recurrence?.interval || 1) || 1
    );
    const dows = initial.daysOfWeek || initial.recurrence?.daysOfWeek || [];
    setDaysOfWeek(
      Array.isArray(dows) ? dows.map((x) => String(x).toUpperCase()) : []
    );
    setErrors({});
  ...
  const delParam = (i) =>
    setReportParams((prev) => prev.filter((_, idx) => idx !== i));
  ...
  const validate = () => {
    ...
    if (!reportId && !reportName.trim())
      e.report = "Select a report or type its name";
    if (!emailRecipients.trim())
      e.emailRecipients = "At least one recipient is required";
    if (!attachmentFormat) e.attachmentFormat = "Attachment format is required";
    if (!startDate) e.startDate = "Start date is required";
    if (!startTime) e.startTime = "Start time is required";
    if (useCron && !cronExpression.trim())
      e.cronExpression = "CRON expression is required";
    if (!useCron) {
      if (!frequency) e.frequency = "Frequency is required";
      if (!interval || Number(interval) <= 0)
        e.interval = "Interval must be greater than 0";
      if (frequency === "WEEKLY" && !daysOfWeek.length)
        e.daysOfWeek = "Pick at least one weekday";
    }
    setErrors(e);
      ...
      <Card>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            ...
            {reportOptions.length ? (
              <select
                ...
                onChange={(e) => {
                  ...
                  setReportName("");
                  if (errors.report) setErrors((x) => ({ ...x, report: "" }));
                }}
                className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
              >
            ...
            {errors.report && (
              <p className="text-xs text-red-500 mt-1">{errors.report}</p>
            )}
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                setAttachmentFormat(e.target.value);
                if (errors.attachmentFormat)
                  setErrors((x) => ({ ...x, attachmentFormat: "" }));
              }}
              className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
            >
              {formatOptions.map((f) => (
        ...
        {!useCron ? (
          <div className="grid md:grid-cols-3 gap-4 mt-4">
            <div>
              ...
              {errors.frequency && (
                <p className="text-xs text-red-500 mt-1">{errors.frequency}</p>
              )}
            </div>
            <div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {weekdayOptions.map((d) => (
                    <label
                      key={d}
                      className="inline-flex items-center gap-2 text-sm bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded-md"
                    >
                      <input
                        type="checkbox"
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Quartz CRON format. Start time is used as the first fire time (if
              applicable).
            </p>
          </div>
        )}
