  ...
  return (
    <div className="grid md:grid-cols-12 gap-2 items-end">
      ...
      <div className="md:col-span-2 flex">
        <Button variant="danger" onClick={onRemove} className="mt-6 w-full">
          Remove
        </Button>
      </div>
...
const JournalEntryForm = ({ onSubmit, submitting, defaultOfficeId }) => {
  ...
  const [officeId, setOfficeId] = useState(
    defaultOfficeId ? String(defaultOfficeId) : ""
  );
  ...
  const totals = useMemo(() => {
    const d = lines.reduce(
      (acc, l) =>
        acc + (l.debitOrCredit === "DEBIT" ? Number(l.amount || 0) : 0),
      0
    );
    const c = lines.reduce(
      (acc, l) =>
        acc + (l.debitOrCredit === "CREDIT" ? Number(l.amount || 0) : 0),
      0
    );
  ...
  const addLine = (type = "DEBIT") => {
    setLines((ls) => [
      ...ls,
      { accountId: "", debitOrCredit: type, amount: "", comments: "" },
    ]);
  };
  ...
  const validate = () => {
    ...
    lines.forEach((l, i) => {
      ...
      if (!l.amount || Number(l.amount) <= 0)
        e[`line_${i}_amount`] = "Positive amount required";
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="space-x-2">
            <Button variant="secondary" onClick={() => addLine("DEBIT")}>
              Add Debit
            </Button>
            <Button variant="secondary" onClick={() => addLine("CREDIT")}>
              Add Credit
            </Button>
          </div>
        </div>
        ...
        <div className="mt-4 grid md:grid-cols-3 gap-2 text-sm">
          <div className="font-medium">
            Total Debits: {totals.debit.toFixed(2)}
          </div>
          <div className="font-medium">
            Total Credits: {totals.credit.toFixed(2)}
          </div>
          <div
        ...
        {errors.balanced && (
          <p className="text-xs text-red-500 mt-1">{errors.balanced}</p>
        )}
      </Card>
