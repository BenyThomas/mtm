...
const HookForm = ({ initial, onSubmit, submitting }) => {
  ...
  const templateForType = useMemo(() => {
    ...
    return (
      templates.find((t) => (t?.name || t?.type || t?.code) === hookType) ||
      null
    );
  }, [tpl, hookType]);
  ...
  const eventOptions = useMemo(() => {
     ...
     */
    const list =
      tpl?.events || tpl?.eventOptions || tpl?.entityActionOptions || [];
    let out = [];
    if (Array.isArray(list)) {
      out = list
        .map((e) => {
          ...
          return ent && act
            ? {
                entityName: String(ent).toUpperCase(),
                actionName: String(act).toUpperCase(),
              }
            : null;
        })
        .filter(Boolean);
      ...
      fallbackEntities.forEach((ent) => {
        fallbackActions.forEach((act) =>
          out.push({ entityName: ent, actionName: act })
        );
      });
    }
  ...
  const suggestedConfig = useMemo(() => {
    ...
    const params =
      templateForType?.configParameters || templateForType?.config || [];
  ...
  const toggleEvent = (e) => {
    ...
    if (has)
      setEvents((prev) =>
        prev.filter((x) => `${x.entityName}:${x.actionName}` !== key)
      );
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (errors.hookType) setErrors((x) => ({ ...x, hookType: "" }));
              }}
            ...
            {errors.hookType && (
              <p className="text-xs text-red-500 mt-1">{errors.hookType}</p>
            )}
          </div>
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <div className="space-x-2">
            ...
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                setEvents(eventOptions.slice(0, 20));
              }}
            >
              Quick Select
            </Button>
        ...
        <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-auto pr-1">
          {eventOptions.map((e, i) => {
            const key = `${e.entityName}:${e.actionName}`;
            const checked = events.some(
              (x) => `${x.entityName}:${x.actionName}` === key
            );
            return (
              <label
                key={key}
              ...
              >
                ...
                />
                <span>
                  <span className="font-mono">{e.entityName}</span> →{" "}
        ...
        {errors.events && (
          <p className="text-xs text-red-500 mt-2">{errors.events}</p>
        )}
      </Card>
          ...
          {config.map((c, i) => {
            ...
            return (
              <div key={i} className="grid md:grid-cols-5 gap-2">
                ...
                {err && (
                  <p className="text-xs text-red-500 md:col-span-5 -mt-1">
                    {err}
                  </p>
                )}
              </div>
