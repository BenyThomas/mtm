...
const EntityDatatableCheckForm = ({ initial, onSubmit, submitting }) => {
  ...
  const entityOptions = useMemo(() => {
    const src =
      template?.entities ?? template?.entityOptions ?? template?.entity ?? [];
    ...
    const norm = list
      .map((x) => {
        ...
        return id && name
          ? { id: String(id).toUpperCase(), name: String(name).toUpperCase() }
          : null;
      })
      .filter(Boolean);
      ...
      if (entity === "LOAN") {
        try {
          ...
          const list = Array.isArray(r.data)
            ? r.data
            : r.data?.pageItems || r.data?.loanProductOptions || [];
          const norm = list
        ...
        try {
          ...
          const list = Array.isArray(r.data)
            ? r.data
            : r.data?.pageItems || r.data?.savingsProductOptions || [];
          const norm = list
            .map((p) => ({ id: p.id, name: p.name }))
            .filter((p) => p.id != null);
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {errors.entity && (
            <p className="text-xs text-red-500 mt-1">{errors.entity}</p>
          )}
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Accepts status code or name. Use the template docs for exact values
            in your tenant.
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {errors.datatableName && (
            <p className="text-xs text-red-500 mt-1">{errors.datatableName}</p>
          )}
        </div>
