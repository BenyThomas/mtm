...
const CashierForm = ({ tellerId, initial, onSubmit, submitting }) => {
  ...
  const [isFullDay, setIsFullDay] = useState(
    Boolean(initial?.isFullDay ?? true)
  );
  const [fromDate, setFromDate] = useState(
    initial?.fromDate ? String(initial.fromDate).slice(0, 10) : ""
  );
  const [toDate, setToDate] = useState(
    initial?.toDate ? String(initial.toDate).slice(0, 10) : ""
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const d = r?.data || {};
        const raw = d?.staffOptions || d?.loanOfficerOptions || d?.staff || [];
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (errors.staffId) setErrors((x) => ({ ...x, staffId: "" }));
                }}
