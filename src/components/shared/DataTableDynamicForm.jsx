...
const DataTableDynamicForm = ({ columns, initial, onSubmit, submitting }) => {
  ...
  useEffect(() => {
    ...
    normCols.forEach((c) => {
      ...
      if (
        (c.lower.includes("date") || c.lower.includes("time")) &&
        Array.isArray(val) &&
        val.length >= 3
      ) {
        const [y, m, d] = val;
        val = `${y}-${String(m).padStart(2, "0")}-${String(d).padStart(
          2,
          "0"
        )}`;
      }
    ...
    (async () => {
      ...
      for (const c of colsWithCode) {
        try {
          ...
          const list = Array.isArray(vals.data)
            ? vals.data
            : vals.data?.pageItems || [];
  ...
  const inputFor = (c) => {
    ...
    if (
      c.lower.includes("decimal") ||
      c.lower.includes("number") ||
      c.lower.includes("int")
    ) {
      return (
        <input
          type="number"
