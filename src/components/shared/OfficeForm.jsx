...
const OfficeForm = ({ initial, onSubmit, submitting }) => {
  ...
  const parentOptions = useMemo(() => {
    const raw =
      tpl?.allowedParents || tpl?.parentIdOptions || tpl?.officeOptions || [];
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium">
              Parent Office {parentOptions.length ? "*" : "(optional)"}
            </label>
            <select
              value={parentId}
              onChange={(e) => {
                setParentId(e.target.value);
                if (errors.parentId) setErrors((x) => ({ ...x, parentId: "" }));
              }}
              className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
            >
              <option value="">
                {parentOptions.length ? "Select parent…" : "—"}
              </option>
              {parentOptions.map((o) => (
