...
const ClientForm = ({ initial, onSubmit, submitting }) => {
  ...
  const [officeId, setOfficeId] = useState(
    initial?.officeId || initial?.office?.id || ""
  );
  ...
  const [dateOfBirth, setDateOfBirth] = useState(
    toISO(
      Array.isArray(initial?.dateOfBirth)
        ? initial.dateOfBirth.join("-")
        : initial?.dateOfBirth
    )
  );
  ...
  const [activationDate, setActivationDate] = useState(
    toISO(
      Array.isArray(initial?.activationDate)
        ? initial.activationDate.join("-")
        : initial?.activationDate
    )
  );

  const [staffId, setStaffId] = useState(
    initial?.staffId || initial?.staff?.id || ""
  );
  const [genderId, setGenderId] = useState(
    initial?.genderId || initial?.gender?.id || ""
  );
  const [clientTypeId, setClientTypeId] = useState(
    initial?.clientTypeId || initial?.clientType?.id || ""
  );
  const [clientClassificationId, setClientClassificationId] = useState(
    initial?.clientClassificationId || initial?.clientClassification?.id || ""
  );
  const [legalFormId, setLegalFormId] = useState(
    initial?.legalFormId || initial?.legalForm?.id || ""
  );
    ...
    (async () => {
      ...
      try {
        ...
        const legal = normalize(
          d?.legalFormOptions || d?.legalFormIdOptions || []
        );
  ...
  useEffect(() => {
    ...
    setClientClassificationId(
      initial?.clientClassificationId || initial?.clientClassification?.id || ""
    );
    setLegalFormId(
      initial?.legalFormId || initial?.legalForm?.id || legalFormId
    );
    setErrors({});
  ...
  const validate = () => {
    ...
    if (active && !activationDate)
      e.activationDate = "Activation date is required for active clients";
    setErrors(e);
  ...
  const submit = async (ev) => {
    ...
    const payload = {
      ...
      ...(clientClassificationId
        ? { clientClassificationId: Number(clientClassificationId) }
        : {}),
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (errors.legalFormId)
                    setErrors((x) => ({ ...x, legalFormId: "" }));
                }}
                className="mt-1 w-full border rounded-md p-2 dark:bg-gray-700 dark:border-gray-600"
              >
