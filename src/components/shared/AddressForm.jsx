...
const AddressForm = ({ clientId, initial, onSubmit, submitting }) => {
  ...
  const [addressTypeId, setAddressTypeId] = useState(
    initial?.addressTypeId || initial?.addressType?.id || ""
  );
  const [isActive, setIsActive] = useState(
    typeof initial?.isActive === "boolean" ? initial.isActive : true
  );
  const [isPrimary, setIsPrimary] = useState(
    typeof initial?.isPrimary === "boolean" ? initial.isPrimary : false
  );
  ...
  const [stateProvinceId, setStateProvinceId] = useState(
    initial?.stateProvinceId || initial?.stateProvince?.id || ""
  );
  const [countryId, setCountryId] = useState(
    initial?.countryId || initial?.country?.id || ""
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const d = r?.data || {};
        const addrTypes = normalize(
          d?.addressTypeOptions || d?.addressTypes || []
        );
  ...
  const validate = () => {
    ...
    if (!addressLine1 && !street)
      e.addressLine1 = "Address Line 1 or Street is required";
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (errors.addressTypeId)
                    setErrors((x) => ({ ...x, addressTypeId: "" }));
                }}
