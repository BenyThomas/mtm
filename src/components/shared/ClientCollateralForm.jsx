...
const ClientCollateralForm = ({
  ...
  const [typeId, setTypeId] = useState(
    initial?.typeId || initial?.collateralTypeId || initial?.type?.id || ""
  );
  const [name, setName] = useState(initial?.name || "");
  const [description, setDescription] = useState(initial?.description || "");
  const [quantity, setQuantity] = useState(initial?.quantity ?? 1);
  const [unitPrice, setUnitPrice] = useState(
    initial?.unitPrice ?? initial?.value ?? ""
  );
  ...
  const hydrateFromTemplate = (tpl) => {
    ...
    const normQual = Array.isArray(qualities)
      ? qualities.map((o) => ({
          id: o?.id ?? o?.value ?? o?.key ?? o,
          name: o?.name ?? o?.text ?? o?.label ?? String(o),
        }))
      : [];

    const normUnits = Array.isArray(unitTypes)
      ? unitTypes.map((o) => ({
          id: o?.id ?? o?.value ?? o?.key ?? o,
          name: o?.name ?? o?.text ?? o?.label ?? String(o),
        }))
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled)
          ToastAlert.error("Failed to load client collateral template");
      } finally {
        ...
        {tplLoading ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              {errors.typeId && (
                <p className="text-xs text-red-500 mt-1">{errors.typeId}</p>
              )}
            </div>
