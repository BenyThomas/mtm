...
const ReportForm = ({ initial, onSubmit, submitting }) => {
  ...
  useEffect(() => {
    ...
    setUseReport(
      initial.useReport != null
        ? Boolean(initial.useReport)
        : initial.use_report != null
        ? Boolean(initial.use_report)
        : true
    );
    ...
    setParams(
      Array.isArray(p) && p.length
        ? p.map((x) => ({
            name: x.name || x.reportParameterName || x.parameterName || "",
          }))
        : [{ name: "" }]
    );
    setErrors({});
  ...
  const typeOptions = useMemo(() => {
    ...
    return Array.isArray(raw)
      ? raw.map((x) => x?.name || x?.value || x).filter(Boolean)
      : [];
  }, [tpl]);

  const subTypeOptions = useMemo(() => {
    const raw =
      tpl?.reportSubTypeOptions || tpl?.subTypeOptions || tpl?.subtypes || [];
    return Array.isArray(raw)
      ? raw.map((x) => x?.name || x?.value || x).filter(Boolean)
      : [];
  }, [tpl]);
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          >
            ...
            {typeOptions.length
              ...
              : ["Table", "Chart", "Pentaho", "Jasper"].map((opt) => (
                  <option key={opt} value={opt}>
                    {opt}
                  </option>
                ))}
          ...
          {errors.reportType && (
            <p className="text-xs text-red-500 mt-1">{errors.reportType}</p>
          )}
        </div>
        ...
        <div>
          ...
          >
            ...
            {categoryOptions.length
              ...
              : ["Loans", "Clients", "Accounting", "Portfolio", "Custom"].map(
                  (opt) => (
                    <option key={opt} value={opt}>
                      {opt}
                    </option>
      ...
      <div>
        ...
        />
        {errors.reportSql && (
          <p className="text-xs text-red-500 mt-1">{errors.reportSql}</p>
        )}
        <p className="text-xs text-gray-500 mt-1">
          Use parameter placeholders if supported by your tenant (e.g.{" "}
        ...
        <div className="mt-3 space-y-2">
          {params.map((p, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              />
              <Button variant="danger" onClick={() => delParam(i)}>
                Remove
              </Button>
            </div>
          ))}
