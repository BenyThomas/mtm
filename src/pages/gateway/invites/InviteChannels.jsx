...
import { createInviteChannel, deleteInviteChannel, listInviteChannels, updateInviteChannel } from '../../../api/gateway/invites';
...
const InviteChannels = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await listInviteChannels({
        q: search || undefined,
      ...
      setRows(items.map((item) => ({ ...item, id: item.inviteChannelId })));
      setTotal(Number(response?.total || items.length || 0));
    } catch (e) {
      setRows([]);
      setTotal(0);
      addToast(e?.response?.data?.message || e?.message || 'Failed to load channels', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editing?.inviteChannelId) {
        await updateInviteChannel(editing.inviteChannelId, payload);
        addToast('Channel updated', 'success');
      } else {
        await createInviteChannel(payload);
  ...
  const confirmDelete = async () => {
    if (!target?.inviteChannelId) return;
    setDeleting(true);
    try {
      await deleteInviteChannel(target.inviteChannelId);
  ...
  return (
    <div className="space-y-4">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold">Invite Channels</h1>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Manage invite channels used by invite creation screens.</p>
        </div>
        <Button onClick={openCreate}><Plus size={16} /> New Channel</Button>
      </section>
      ...
      >
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Delete channel <span className="font-medium text-slate-900 dark:text-slate-100">{target?.name || target?.code || '-'}</span>?
        </p>
      </Modal>
    </div>
  );
